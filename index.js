...
async function getWeatherData(){
    let responseData =await fetch(`https://api.weatherapi.com/v1/forecast.json?key=71dbd040c45b418aa92175158231802&q=london&days=3`)
...
function getNextData(data){
 let dataForcast = data.forecast.forecastday
 for(let i =0 ; i<2 ; i++){
    let nextDate = new Date(dataForcast[i+1].date)
    nextDay[i].innerHTML = nextDate.toLocaleDateString('en-US',{weekday:'long'})
    nextMaxTemp[i].innerHTML = dataForcast[i+1].day.maxtemp_c
    nextMinTemp[i].innerHTML = dataForcast[i+1].day.mintemp_c
    nextConditionImg[i].setAttribute("src",dataForcast[i+1].day.condition.icon)
    nextConditionText[i].innerHTML = dataForcast[i+1].day.condition.text
 }
